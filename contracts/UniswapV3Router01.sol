...
contract UniswapV3Router01 is IUniswapV3Router01 {
    ...
    function swapTokensForTokens(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
        external
        virtual
        payable
        ensure(deadline)
    ...
    {
        require(path.length >= 4, 'UniswapV2Router: PATH TOO SMALL');
        require(path[0] == path[path.length - 1], 'UniswapV2Router: INVALID_PATH');
